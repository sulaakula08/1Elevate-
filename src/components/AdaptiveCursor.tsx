...
// These are text-bearing semantic elements, rather than every span or div that
// happens to contain a text node. `data-cursor="text"` is the explicit escape
// hatch for unusual prose markup.
const TEXT_SELECTOR = [
  "h1",
  "h2",
  "h3",
...
/**
 * Landing-only pointer replacement modelled on Motion's adaptive Cursor.
 * Position is a direct MotionValue write; only the shape interpolates.
 */
export function AdaptiveCursor() {
  ...
  useEffect(() => {
    ...
    const startTracking = () => {
      ...
      const textCaretHeight = (element: Element) => {
        ...
        // Motion's reference caret resolves to the glyph size, while line-height
        // remains a guard for unusually tight type. Clamp decorative display
        // sizes so the pointer can never become a full-height bar.
        const height = Math.round(Math.min(72, Math.max(12, Math.min(fontSize, lineHeight))));
        textMetrics.set(element, height);
        return height;
      };
      ...
      const updateTarget = (target: Element | null) => {
        ...
        if (target) {
          ...
          // A disabled control is still recognised, but deliberately never gets
          // the inviting expanded state of an enabled control.
          if (disabled) {
        ...
        const shape = next.kind === "text"
          ? { width: 4, height: next.height }
          : SHAPES[next.kind];
  ...
  return (
    <motion.div
      aria-hidden="true"
      className="adaptive-cursor-shell"
      style={{ x, y, opacity }}
    >
      <motion.div
        ref={cursor}
