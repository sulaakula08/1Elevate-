-- ===========================================================================
-- BASELINE MIGRATION — the schema as it already stands in production.
--
-- This is a byte-for-byte copy of supabase/schema.sql at the commit that
-- introduced migrations, and it exists to give the migration history a floor to
-- stand on. Everything in here was applied to production by hand, through the
-- SQL Editor, before there was any migration history at all.
--
-- Which means the two databases get here by two different routes, on purpose:
--
--   * A NEW project (1elevate-dev) runs it. `supabase db push` executes this
--     file and the database ends up at the current shape from empty.
--
--   * PRODUCTION does not run it. It is already in this state, so it is marked
--     as applied without being executed:
--
--       npx supabase migration repair --status applied 20260811000000
--
--     That is the official CLI workflow for adopting migrations on a database
--     that predates them, and it is the safe one. Re-running this file against
--     production would in fact survive — every statement is guarded, and the
--     idempotency was verified statement by statement before this file was
--     created — but two of the statements are not purely structural:
--
--       * the de-duplicating `delete from public.attempts` before the unique
--         index, which removes rows that are byte-identical repeats;
--       * `delete from public.custom_questions where trim(id) = ''`.
--
--     Both are no-ops on healthy data and both are there for good reasons, but
--     "no-op on healthy data" is not a thing to assert about a live student
--     database when `migration repair` makes the question moot.
--
-- From here on, DO NOT EDIT THIS FILE and do not edit schema.sql. Every change
-- is a new timestamped migration beside this one. See DATABASE.md.
-- ===========================================================================
